...
from visualize import render_gif_from_pc, render_gif_from_mesh, render_gif_from_voxel
...
def evaluate_model(args):
    ...
    if args.load_checkpoint:
        checkpoint = torch.load(f'checkpoint_{args.type}.pth')  #_{name}
        # checkpoint = torch.load(f'test4_model2_checkpoints/checkpoint_{args.type}.pth')
    ...
    for step in range(start_iter, max_iter):
        ...
        if (step % int(args.vis_freq)) == 5:
            plt.imsave(f'{step}_{args.type}.png', images_gt.squeeze(). detach().cpu().numpy())
            print("Rendering and saving Ground Truth")
            render_gif_from_mesh(mesh= mesh_gt, output_path='results/q2_mesh_GT.gif')

            if(args.type == "point"):
                print("Rendering and saving predicted point cloud")
                # import pdb; pdb.set_trace()
                render_gif_from_pc(predictions, output_path=f'results/q2_point_cloud_fit.gif') #_{name}
            elif(args.type == "vox"):
                print("Rendering and saving predicted voxel")
                pred = predictions.squeeze(0)
                render_gif_from_voxel(voxel = pred, output_path='results/q2_voxel_fit.gif', distance= 3.0 )
            elif(args.type == "mesh"):
                print("Rendering and saving predicted mesh")
                render_gif_from_mesh(mesh= predictions, output_path='results/q2_mesh_fit.gif', distance = 1.0)
            # break
        ########################################################################################
